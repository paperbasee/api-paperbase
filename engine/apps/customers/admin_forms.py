...
def build_customer_extra_form_fields(schema: list[dict[str, Any]]) -> dict[str, forms.Field]:
    """
    Build Django form fields for customer extra_field_schema items.

    Django admin validates `fieldsets` against *form.base_fields* at form-class construction time.
    So these fields must be attachable to the form class (not only in __init__).
    """
...
class CustomerAdminForm(forms.ModelForm):
    """Adds one form field per customer extra_field_schema row (same keys in extra_data as dashboard)."""
    ...
    def _resolve_store_and_build_extra_fields(self) -> None:
        ...
        self._extra_schema = get_customer_extra_schema(store)
        ...
        for item in self._extra_schema:
            ...
            if fname not in self.fields:
                self.fields.update(build_customer_extra_form_fields([item]))
