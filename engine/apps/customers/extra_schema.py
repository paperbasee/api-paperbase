"""
Resolve per-store extra field definitions for customers (dashboard dynamic fields).
...
def get_customer_extra_schema(store: Store | None) -> list[dict[str, Any]]:
    """Return sorted customer entity field definitions for this store."""
    if store is None:
        return []
    try:
    ...
    for item in raw:
        if not isinstance(item, dict):
            continue
        et = item.get("entityType") or item.get("entity_type") or "customer"
        if et != "customer":
            continue
