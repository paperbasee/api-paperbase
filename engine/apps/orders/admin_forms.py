...
def build_order_extra_form_fields(schema: list[dict[str, Any]]) -> dict[str, forms.Field]:
    """
    Build Django form fields for order extra_field_schema items.
...
class OrderAdminForm(forms.ModelForm):
    """Adds one form field per order extra_field_schema row (same keys in extra_data as dashboard)."""
    ...
    def _resolve_store_and_build_extra_fields(self) -> None:
        ...
        self._extra_schema = get_order_extra_schema(store)
        ...
        for item in self._extra_schema:
            ...
            if fname not in self.fields:
                self.fields.update(build_order_extra_form_fields([item]))
